...
import {cleanCard, decrementItemPizza, incrementItemPizza, removeItemCard} from "../../redux/actions/card";
...
const Card = () => {
    ...
    const cleanCardHandler = () => {
        let questionConfirm = window.confirm("Вы действительно хотите очистить корзину ?");
    ...
    const removeItemHandler = (id) => {
        let questionConfirm = window.confirm("Вы действительно удалить пиццу ?");

        if (questionConfirm) {
            dispatch(removeItemCard(id))
        }
    }

    const incrementPizzaItemHandler = (id) => {
        dispatch(incrementItemPizza(id))
    }

    const decrementPizzaItemHandler = (id) => {
        dispatch(decrementItemPizza(id))
    }
            ...
            <div className="cart">
                <div className="cart__top">
                    <h2 className="content__title"><svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        Корзина</h2>
                    <div className="cart__clear" onClick={cleanCardHandler}>
...

                {
                    addedPizzas.map((obj, index) => <CartItem
                        ...
                        type={obj.items[0].types}
                        size={obj.items[0].size}
                        totalPrice={obj.totalPrice}
                        totalCount={obj.items.length}
                        removeItemHandler={removeItemHandler}
                        incrementPizzaItem={incrementPizzaItemHandler}
                        decrementPizzaItem={decrementPizzaItemHandler}
                    />)
                }
